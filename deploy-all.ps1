...
if (-not $SkipUserCreation) {
    ...
    try {
        ...
        Write-Host "⚠ Admin user creation failed (may already exist)" -ForegroundColor Yellow
...
if (Test-Path ".env") {
    ...
    if ($envContent -match "VITE_USER_POOL_ID=us-east-1_") {
        ...
        Write-Host "⚠ Environment variables may not be fully configured" -ForegroundColor Yellow
    }
} else {
...
if (Test-Path "src/aws-exports.ts") {
    ...
    Write-Host "✗ aws-exports.ts not found" -ForegroundColor Red
}
...
if (Test-Path "deployment-info.json") {
    ...
    Write-Host "⚠ Deployment info not found" -ForegroundColor Yellow
}
...
Write-Host "   (You'll be prompted to change it)" -ForegroundColor Gray
Write-Host ""
...
Write-Host "  - Deployment Guide: DEPLOYMENT_STEPS.md" -ForegroundColor Gray
